...
try:
    ...
    infinity_table.drop_index('CQADupStack_en_ft_index',ConflictType.Ignore)
    begin_full_text_time = time.time()
    res = infinity_table.create_index("CQADupStack_en_ft_index",
                                            index.IndexInfo("fulltext_col", index.IndexType.FullText, ft_params),
                                            ConflictType.Error)
    infinity_table.show_index("CQADupStack_en_ft_index")
    end_full_text_time = time.time()
    print(infinity_table.show_index('CQADupStack_en_ft_index'))
