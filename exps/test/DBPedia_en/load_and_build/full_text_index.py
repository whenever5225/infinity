...
try:
    ...
    hybridsearch_table = hybridsearch_db.get_table("DBPedia_en_Table")
    hybridsearch_table.drop_index('DBPedia_en_ft_index',ConflictType.Ignore)
    begin_full_text_time = time.time()
    res = hybridsearch_table.create_index("DBPedia_en_ft_index",
                                            index.IndexInfo("fulltext_col", index.IndexType.FullText, ft_params),
                                            ConflictType.Error)
    hybridsearch_table.show_index("DBPedia_en_ft_index")
    end_full_text_time = time.time()
    print(hybridsearch_table.show_index('DBPedia_en_ft_index'))
