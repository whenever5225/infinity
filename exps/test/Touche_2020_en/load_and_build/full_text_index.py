...
from Touche_2020_en.search_tensor_rank.three_roads.sparse_dense_fulltext_search import read_file_content, terminate_process_tree
...
try:
    ...
    hybridsearch_table = hybridsearch_db.get_table("Touche_2020_en_Table")
    hybridsearch_table.drop_index('Touche_2020_en_ft_index',ConflictType.Ignore)
    begin_full_text_time = time.time()
    res = hybridsearch_table.create_index("Touche_2020_en_ft_index",
                                            index.IndexInfo("fulltext_col", index.IndexType.FullText, ft_params),
                                            ConflictType.Error)
    hybridsearch_table.show_index("Touche_2020_en_ft_index")
    end_full_text_time = time.time()
    print(hybridsearch_table.show_index('Touche_2020_en_ft_index'))
